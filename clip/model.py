...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)    #[B, 3, 224, 224] --> [B, 768, 14, 14]
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.ln_post(x[:, 0, :])    # class_token
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer(
            ...
            attn_mask=self.build_attention_mask()       # attn_mask：
                                                        #   [[ 0, -inf, -inf, -inf],
                                                        #    [ 0,   0, -inf, -inf],
                                                        #    [ 0,   0,   0, -inf],
                                                        #    [ 0,   0,   0,   0]]
    ...
    def encode_text(self, text):
        ...
        # torch.arange(x.shape[0]) :  class number  [0, 1, 2, 3, 4, 5,..., n_cls - 1] [n_cls]
        # text.argmax(dim=-1)      :[n_cls]
        # x[torch.arange(x.shape[0]), text.argmax(dim=-1)]:  [n_cls, 77, 512] --> [n_cls, 1, 512] --> [n_cls, 512]

        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection     # [n_cls, 512] * [512, 512] = [n_cls, 512]
