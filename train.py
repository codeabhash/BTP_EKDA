...
print("Experimental start time: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def reset_cfg(cfg, args):
    ...
    if args.kd:
        ...
        if args.trainer == "EKDA":
            cfg.TRAINER.EKDA.KD_WEIGHT = float(args.kd)  # fix: --kd was ignored by the loss
...
def extend_cfg(cfg, args):
    ...
    elif args.trainer == 'EKDA':
        cfg.TRAINER.EKDA = CN()
        cfg.TRAINER.EKDA.PREC = "fp16"
        cfg.TRAINER.EKDA.DROPOUT = 0.0

        cfg.TRAINER.EKDA.TP = True
        cfg.TRAINER.EKDA.T_DEEP = True
        cfg.TRAINER.EKDA.VP = True
        cfg.TRAINER.EKDA.V_DEEP = True
        cfg.TRAINER.EKDA.CSC = False
        cfg.TRAINER.EKDA.N_CTX = 4  # number of text context vectors
        cfg.TRAINER.EKDA.CTX_INIT = "a photo of a"
        cfg.TRAINER.EKDA.CLASS_TOKEN_POSITION = "end"

        cfg.TRAINER.EKDA.LOCATION = "middle"
        cfg.TRAINER.EKDA.KD_WEIGHT = 100.0
        cfg.TRAINER.EKDA.TEMPERATURE = 1.0
        cfg.TRAINER.EKDA.TEACHER_NAME = "ViT/L-14"
        cfg.TRAINER.EKDA.NUM_TOKENS = cfg.TRAINER.EKDA.N_CTX  # number of visual context vectors

        cfg.TRAINER.EKDA.TEXT_LOSS_WEIGHT = 25
        cfg.TRAINER.EKDA.IMAGE_LOSS_WEIGHT = 10
...
if __name__ == "__main__":
    ...
    proc_title = args.trainer
    setproctitle.setproctitle(proc_title)

    main(args)
    print("Experimental finish time: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
