...
def load_clip_to_cpu(cfg, teacher_model=False):
    if teacher_model:
        if cfg.TRAINER.NAME =="EKDA":
            backbone_name = cfg.TRAINER.EKDA.TEACHER_NAME
        elif cfg.TRAINER.NAME =="SFEKDA":
            backbone_name = cfg.TRAINER.SFEKDA.TEACHER_NAME
    else:
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, deep_prompts=None, context=False):
        ...
        return text_embedding_at_eos
    
    def transformer_deep(self, x, deep_x):
        if self.deep:
            if deep_x.dim == 2:
                if self.deep_layer == None:
                    deep_x = deep_x.unsueeze(0).expand(self.num_layer - 1, -1, -1)  # all layers exsit ekda
                else:
                    deep_x = deep_x.unsueeze(0).expand(self.deep_layer[1] - self.deep_layer[0] + 1, -1, -1) # only specified layers exsit ekda
        ...
        for i in range(self.num_layer):
            if i == 0:
                ...
                if self.deep:
                    if self.deep_layer == None:   # only specified layers exsit ekda
                        if i <= deep_x.shape[0]:   
...
class ImageEncoder_Conv(nn.Module):
    ...
    def forward(self, x, vctx=None, deep_vctx=None, return_feat=False):
        '''
        Maybe futherly there will be some ekda tuning methods for convolutional networks
        '''
        def stem(x):
...
class ImageEncoder_Trans(nn.Module):
    ...
    def forward(self, x, vctx=None, deep_vctx=None, attn_map=False):
        # x [B, 3, 224, 224]
    ...
    def incorporate_prompt(self, x, vctx):
        # combine ekda embeddings with image-patch embeddings
